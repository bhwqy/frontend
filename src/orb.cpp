...
static float IC_Angle(const cv::Mat& image, cv::Point2f pt, const std::vector<int>& u_max) {
    int m_01 = 0, m_10 = 0;
    const uchar* center = &image.at<uchar>(int(std::round(pt.y)), int(std::round(pt.x)));
...
static void computeOrbDescriptor(const cv::KeyPoint& kpt, const cv::Mat& img, const cv::Point* pattern, uchar* desc) {
    ...
    const uchar* center = &img.at<uchar>(int(std::round(kpt.pt.y)), int(std::round(kpt.pt.x)));
    const int step = (int)img.step;

    auto GET_VALUE = [&](int idx) -> uchar {
        int centeridx = int(std::round(pattern[idx].x * b + pattern[idx].y * a)) * step
            + int(std::round(pattern[idx].x * a - pattern[idx].y * b));
        return center[centeridx];
    };
    ...
    iniThFAST(_iniThFAST), minThFAST(_minThFAST)
{
    ...
    for (int level = 0; level < nlevels-1; level++) {
        mnFeaturesPerLevel[level] = std::round(nDesiredFeaturesPerScale);
    ...
    int v, v0, vmax = std::floor(HALF_PATCH_SIZE * std::sqrt(2.f) / 2 + 1);
    int vmin = std::ceil(HALF_PATCH_SIZE * std::sqrt(2.f) / 2);
    const double hp2 = HALF_PATCH_SIZE * HALF_PATCH_SIZE;
    for (v = 0; v <= vmax; ++v)
        umax[v] = std::round(std::sqrt(hp2 - v * v));
...
static void computeDescriptors(
    const cv::Mat& image, std::vector<cv::KeyPoint>& keypoints, cv::Mat& descriptors, const std::vector<cv::Point>& pattern
) {
