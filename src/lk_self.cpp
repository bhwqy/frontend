...
int main(int argc, char **argv) {
    ...
    if (argc < 3) {
        std::cout << "usage: lk_self img1 img2 ...\n";
        return 1;
    }
    ...
    std::vector<std::vector<bool>> status(n - 1);

    auto t1 = chrono::steady_clock::now();
    for (int i = 0; i < n - 1; ++i)
        OpticalFlowMultiLevel(imgs[i], imgs[i + 1], pts[i], pts[i + 1], status[i], true);
    auto t2 = chrono::steady_clock::now();
    auto time_used = chrono::duration_cast<chrono::duration<double>>(t2 - t1);
    cout << "optical flow by self: " << time_used.count() << endl;
    ...
    for (int i = 0; i < n; ++i) {
        ...
        string s = "../result/self_lk" + std::to_string(i) + ".bmp";
